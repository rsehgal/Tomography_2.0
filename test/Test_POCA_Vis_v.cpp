...
int main(int argc, char *argv[]) {
  ...
  Tomography::VisualizationHelper *vis = new Tomography::VisualizationHelper;
  vis->InitializeVisualizer();
  vis->Register("geom.gdml");
...
// For Incoming Track
  ...
  std::vector<vecgeomVec_t> pocaVec_v;
  //---------- VEctorized Stuff -------
  {
    ...
    // std::cout << "--------------------------------------" << std::endl;
    vecgeomVec_t poca_v = POCA(Pi1, dirI, Po1, dirO);
  ...
  //MyVec_t tempPt;
  for (unsigned int j = 0; j < pocaVec_v.size(); j++) {
    // tempPt.Set(poca->GetX(), poca->GetY(), poca->GetZ());
    vecgeomVec_t pt = pocaVec_v[j];
    for (unsigned int k = 0; k < vecCore::VectorSize<Real_v>(); k++) {
      //tempPt.Set(pocaVec[j].x()[k], pocaVec[j].y()[k], pocaVec[j].z()[k]);
      //tempPt.Set(pt.x()[k], pt.y()[k], pt.z()[k]);
